...
CTAWM.printWhoTable = function( fn, dbg )
	...
	for name, _ in pairs(CTAWM.whoTable) do
...
CTAWM.hookedFriendsFrame_OnEvent = function(this, event, ...) 
	...
	--if( event ~= "WHO_LIST_UPDATE" or ( whoQueue[1] == nil and CTAWM.request == nil ) ) then
		CTAWM.preHookFriendsFrame_OnEvent( this, event, ... );
	end
end
...
CTAWM.pruneWhoTable = function()
	...
	-- remove old entries
	local now = time();
	for name, whoData in pairs(CTAWM.whoTable) do
