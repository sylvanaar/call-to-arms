...
--[[
	...
	Returns:	Number encoding class specified by class name (1-9)
...
CTA_Util.getClassString = function( classSet )
	...
	while( pos > 0 ) do
		if( string.sub(b, pos, pos) == "1" ) then 	
			t = t..CTA_Classes[ CTA_Classes[(CTA_NUM_CLASSES+1)-pos] ].localName.." ";
		end
		pos = pos - 1;
	end
	return t;
...
local function WeightScore(lfx, goal, class, level)
    return lfx*1.5 + goal + class + level*0.5
end
...
--			CTA_Util.logPrintln("Filtered message: ".. msg, "R")
	...
	for _ , word in pairs(CTA_TRIGGER_LIST.LFM) do
		if string.find(msgnew, word) then
	...
	for _ , word in pairs(CTA_TRIGGER_LIST.LFG) do
		if string.find(msgnew, word) then
			lfx_score = lfx_score +1
			slfx_score = 1; 
			searchtype = "D"
	...
	for _ , instance in pairs(CTA_TRIGGER_LIST.GOAL) do
